...
import           HA.Resources.Castor (Enclosure(..), Slot(..))
...
data Options
  = DrivePresence String Slot Bool Bool
  | DriveStatus   String Slot String
  | DriveNew      String String
  deriving (Eq, Show)

run :: [NodeId] -> Options -> Process ()
run nids (DriveStatus serial slot@(Slot enc _) status) =
...
run nids (DrivePresence serial slot@(Slot enc _) isInstalled isPowered) =
