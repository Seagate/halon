...
findRealObjsInPVer rg pver = let
    rackvs = G.connectedToU pver M0.IsParentOf rg :: [M0.RackV]
    racks  = rackvs >>= \x -> (maybeToList $ G.connectedFrom1 M0.IsRealOf x rg :: [M0.Rack])
    enclvs = rackvs >>= \x -> (G.connectedToU x M0.IsParentOf rg :: [M0.EnclosureV])
    encls  = enclvs >>= \x -> (maybeToList $ G.connectedFrom1 M0.IsRealOf x rg :: [M0.Enclosure])
    ctrlvs = enclvs >>= \x -> (G.connectedToU x M0.IsParentOf rg :: [M0.ControllerV])
    ctrls  = ctrlvs >>= \x -> (maybeToList $ G.connectedFrom1 M0.IsRealOf x rg :: [M0.Controller])
    diskvs = ctrlvs >>= \x -> (G.connectedToU x M0.IsParentOf rg :: [M0.DiskV])
    disks  = diskvs >>= \x -> (maybeToList $ G.connectedFrom1 M0.IsRealOf x rg :: [M0.Disk])
...
findFailableObjs rg fs = let
    ...
    encls = racks >>= \x ->
             (G.connectedToU x M0.IsParentOf rg :: [M0.Enclosure])
    ctrls = encls >>= \x ->
             (G.connectedToU x M0.IsParentOf rg :: [M0.Controller])
    disks = ctrls >>= \x ->
             (G.connectedToU x M0.IsParentOf rg :: [M0.Disk])
  in S.unions . fmap S.fromList $
    [ fmap M0.fid racks
    , fmap M0.fid encls
    , fmap M0.fid ctrls
...
findCurrentFailedDevices rg fs = let
    ...
    racks = G.connectedToU fs M0.IsParentOf rg :: [M0.Rack]
    encls = racks >>= \x -> (G.connectedToU x M0.IsParentOf rg :: [M0.Enclosure])
    ctrls = encls >>= \x -> (G.connectedToU x M0.IsParentOf rg :: [M0.Controller])
    disks = ctrls >>= \x -> (G.connectedToU x M0.IsParentOf rg :: [M0.Disk])
...
findMatchingPVer rg fs failedDevs = let
    ...
    allPvers = [ (pver, findRealObjsInPVer rg pver)
                  | pool <- G.connectedToU fs M0.IsParentOf rg :: [M0.Pool]
                  , M0.fid pool /= M0.f_mdpool_fid fs
                  , pver <- G.connectedToU pool M0.IsRealOf rg :: [M0.PVer]
                  ]
