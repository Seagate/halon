...
sendLedUpdate status host sd@(Cas.StorageDevice_XXX1 (T.pack -> sn)) = do
  ...
  let mnode = listToMaybe $ G.connectedTo host Runs rg -- XXX: try all nodes
      modifyLedState mledSt = case G.connectedTo sd Has rg of
        Just slot@Cas.Slot{} -> modifyGraph $ case mledSt of
...
ruleMonitorDriveManager = defineSimpleIf "sspl::monitor-drivemanager" extract $ \(uuid, nid, srdm) -> do
  let enc' = Cas.Enclosure . T.unpack
    ...
    in do
      ...
      case mhost of
        ...
        Just host -> do
              ...
              Nothing -> do
                ...
                runMaybeT $ do
                  dev <- MaybeT $ findHostStorageDevices host
                        >>= filterM (flip StorageDevice.hasIdentifier $ Cas.DIRaidIdx idx)
                        >>= \case
                          [] -> do
          ...
          isReassembling <- G.isConnected host Cas.Is Cas.ReassemblingRaid <$> getLocalGraph
