...
import           HA.RecoveryCoordinator.Castor.Rules (goRack_XXX0)
...
mkHpiTest mkTestRule test transport pg = rGroupTest transport pg $ \pid -> do
    ...
    sayTest $ show iData
    (ls',_)  <- run ls $ do
            mapM_ goRack_XXX0 (id_racks_XXX0 iData)
  ...
  where
    ...
    test rc = do
      ...
      let (enc, serial', idx, devid, wwn, _sdev) =
            if hpiIsNew
            then (pack e, serial++"new", 1, pack $ "/dev/loop" ++ ide ++"_new", pack $ "wwn" ++ ide ++"_1new", R.StorageDevice_XXX1 $ serial++"new")
            else (pack e, serial, 1, pack $ "/dev/loop" ++ ide ++ "_1", pack $ "wwn" ++ ide ++ "_1", R.StorageDevice_XXX1 serial)
