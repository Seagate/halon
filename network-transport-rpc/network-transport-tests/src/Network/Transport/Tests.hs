...
import Network.Transport.Internal ( tryIO, timeoutMaybe)
...
import Data.List (permutations, sortBy,delete)
...
testCloseReopen transport numPings = do
  ...
  forkTry $ do
    ...
    let eventss = sortBy (compare `on` head . head . snd) eventss'
...
testCloseTwice transport numRepeats = do
  ...
  forkTry $ do
    ...
    replicateM numRepeats $ do
      ...
      let check ops rs (ev:evs) = case ev of
            ConnectionOpened cid _ _
                | not (elem cid ops) -> check (cid:ops) rs evs
            ConnectionClosed cid
                | elem cid ops -> check (delete cid ops) rs evs
            Received cid ["ping"]
                | elem cid ops && not (elem cid rs) -> check ops (cid:rs) evs
            _ -> do
               print ev
               True <- return False
               return ()
          ...
          check ops rs evs = do
               ...
               True <- return False
               return ()
      evs <- replicateM 5 $ receive endpoint
      check [] [] evs
...
testConnectToSelfTwice transport numPings = do
  ...
  forkTry $ do
    ...
    True <- return $ events1 == replicate numPings ["pingA"] && events2 == replicate numPings ["pingB"]
                  || events2 == replicate numPings ["pingA"] && events1 == replicate numPings ["pingB"]
...
testCloseEndPoint transport _ = do
  ...
  forkTry $ do
    ...
    do
      ...
      -- ErrorEvent (TransportError (EventConnectionLost addr') _) <- return res
...
testTransport newTransport = do
  ...
  runTests
    ...
    -- , ("ParallelConnects",      testParallelConnects transport 15)
    , ("SendAfterClose",        testSendAfterClose transport 20)
    , ("Crossing",              testCrossing transport 10)
    , ("CloseTwice",            testCloseTwice transport 20)
