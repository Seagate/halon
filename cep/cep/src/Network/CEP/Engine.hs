...
emptyMachine s =
    ...
    , _machRuleCount      = 0
    , _machPhaseBuf       = fifoBuffer ("emptyMachine:" ++ show (__LINE__ :: Int)) Unbounded
...
cepCruise !st req@(Run t) =
    case t of
      ...
      IncomingXXX (uuid, m) -> do
        ...
        let smdataXXX (SMData smid rkey rdata) = "smId=" ++ show (getSMId smid) ++ " rkey=" ++ show rkey ++ " rd=" ++ _ruleDataName rdata
