...
main = hspec $ do
    ...
    describe "nodesByAttribute" $ do
      it "finds the same nodes as query" $ do
        db <- readDB testFile
        nodesByAttribute "m0_station" db `shouldBe` query "m0_station" db
      it "finds none of the same nodes as the opposite query" $ do
        db <- readDB testFile
        shouldSatisfy
          (Vector.toList (nodesByAttribute "m0_station" db) `intersect`
           Vector.toList (query (Neg "m0_station") db))
          null
    ...
    describe "attributesByNode" $ do
      it "m0_all has no value" $ do
        db <- readDB testFile
        Genders.lookup "m0_all" (attributesByNode "pg155-n10" db) `shouldBe` Nothing

      it "finds the minimum receive queue length" $ do
        db <- readDB testFile
        Genders.lookup "m0_min_recv_q" (attributesByNode "pg155-n10" db) `shouldBe` Just "2"
