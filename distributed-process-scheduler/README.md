...
that makes it possible to write randomized tests for programs using the
...
A transition is either of the following actions:
...
tested application produce requests for transitions. There is a scheduler
process that collects these requests and chooses which transition to do next,
then it communicates the decision to the process that is affected by the
chosen transition and the other processes remain blocked. Thus, an execution
is uniquely identified by the seed used to initialize the random number
generator.
...
The transition requests of type (1) are produced whenever a
process sends a message. But neither the sender blocks waiting for the
transition to happen nor does the receiver process unblock.
...
This state is initialized with the value of the first operand. The server is
also told to terminate after serving `n+1` read requests of the state.

Then the function creates as many processes as the second operand indicates,
...
The line featuring the call to `threadDelay` has the function wait between
...
scheduler since we send a message to the process reading the `MVar` to indicate
that the `MVar` is filled. The consumer process in turn, blocks with `expect`
until such a message arrives. `schedulerIsEnabled` is a constant exported by
`distributed-process-scheduler` that indicates whether the scheduler is enabled.
Thus, blocking on the MVar is completely avoided when using the scheduler.

The conditional `when schedulerIsEnabled` could be dropped so the guarded code
